import { CHAINS, TOKEN_ACTIONS, TOKEN_LOGOS, getChainById } from './config/assets.js?v=2026-05-09-live-value-audit-fix';
import { DEFAULT_PAGE_ID, PAGES, TOKEN_DESCRIPTIONS, getPageById } from './config/pages.js?v=2026-05-09-live-value-audit-fix';
import { combinedSupplyApy, maxRoeFromSupplyBorrowAndMultiplier, netApyFromSupplyAndBorrow } from './apyMath.js?v=2026-05-09-live-value-audit-fix';
import {
  irmBorrowApyAtUtilization,
  kinkIrmChartPoints,
  ltvMarkerPercent,
} from './riskVisuals.js?v=2026-05-09-live-value-audit-fix';
import {
  ...
  mergeApySources,
  readStoredApy,
} from './defillama.js?v=2026-05-09-live-value-audit-fix';
import {
  borrowFromMarket,
...
} from './eulerLive.js?v=2026-05-09-live-value-audit-fix';
import {
  ...
  formatAmount,
  readSimulationState,
} from './simulation.js?v=2026-05-09-live-value-audit-fix';
import {
  LIQUIDATION_CHAINS,
  LIQUIDATION_RISK_STORAGE_KEY,
  readStoredLiquidationState,
  refreshLiquidationRiskDashboard,
} from './liquidationRisk.js?v=2026-05-09-live-value-audit-fix';
import {
  ...
  readLiveMetricsJson,
} from './liveMetricsStore.js?v=2026-05-09-live-value-audit-fix';
...
const LIVE_METRICS_MAX_AGE_MS = 30_000;
...
function marketSafetyStatus(positionLtv, liquidationLtv) {
  ...
  if (!liq) return { label: 'Pending', className: 'watch', copy: 'Live LTV limits are pending from the market contract.' };
...
function explorerAddressLink(row, address, label = shortAddress(address)) {
  if (!address || !/^0x[0-9a-fA-F]{40}$/.test(address)) return label || 'Pending';
...
function renderPortfolioPosition(page) {
  ...
  const debtPrice = position?.debtPriceFormatted || position?.debtOraclePriceFormatted || 'N/A';
  const collateralPrice = position?.collateralPriceFormatted || position?.collateralOraclePriceFormatted || 'N/A';
  ...
  const liquidationPrice = position?.liquidationPriceFormatted || 'N/A';
  return `
...
function currentSupplyApyTotal(page) {
  ...
  return lending === 'N/A' ? 'N/A' : lending;
}
...
function earnSupplyApyFromAllocations(page) {
  ...
  const rows = allocations.map((allocation) => {
    ...
    return {
      ...
      supplyApy: market ? contractValue(market, 'borrowApy') : 'N/A',
    };
  });
...
function displayMaxRoe(page) {
  ...
  return number !== null && number < 0 ? '0.00%' : calculated;
}
...
function dollarDisplay(value) {
  ...
  if (!text || isUnresolvedDisplayValue(text)) return 'N/A';
...
function exposureRowApy(page, row) {
  if (!row.pageId) return isUnresolvedDisplayValue(row.apy) || row.apy === '0.00%' ? 'N/A' : row.apy;
  const market = getPageById(row.pageId);
  if (!market) return 'N/A';
  const liveBorrowApy = contractValue(market, 'borrowApy');
  return isUnresolvedDisplayValue(liveBorrowApy) || liveBorrowApy === '0.00%' ? 'N/A' : liveBorrowApy;
}
  ...
  return `
    ...
    <div class="page-grid earn-layout">
      <div class="left-stack">
        ...
        ${renderCard('Addresses', `
          <div class="stat-list">
            ${metric(`${page.asset} token`, assetAddress ? addressLink(page, assetAddress) : 'Pending')}
            ${metric(`${page.asset} vault`, page.contractAddress ? addressLink(page, page.contractAddress) : 'Pending')}
            ...
            <tbody>
              ${riskRows.length ? riskRows.map((row) => `
                <tr>
                  ...
                  <td data-label="owner wallet">${row.owner ? explorerAddressLink(row, row.owner) : 'Pending'}</td>
      ...
      <div class="left-stack">
        ...
        ${renderCard('Overview', `
          <div class="metric-grid">
            ...
            ${metric('Risk manager', page.subtitle)}
...
function renderDebtTokenSubview(page, notice) {
  ...
  const debtMarkets = marketsForDebt(page);
  const debtPrice = contractValue(page, 'debtPrice') || '$1.00';
  return `
    ${renderPageTitle(page)}
    <div class="page-grid market-layout">
    ...
    <div class="cy-home">
      <div class="cy-home-card">
        ...
        <div class="cy-home-actions">
          <a href="#/ipor-crvusd-lp-vault">
            <span>cy-crvUSD IPOR Vault</span>
          </a>
          <a href="#/explore">
          ...
          <aside class="ipor-action">
            ...
            <div class="ipor-action-card">
              ...
              <div class="tx-row ipor-buttons">
                <button class="${actionButtonClass(page.id, 'supply', 'deposit', 'deposit')}" data-live-deposit="${page.id}" data-live-role="ipor" data-live-vault="${page.contractAddress}" data-live-field-name="ipor-amount">Approve / Deposit</button>
                <button class="${actionButtonClass(page.id, 'supply', 'withdraw', 'deposit')}" data-live-withdraw="${page.id}" data-live-role="ipor" data-live-vault="${page.contractAddress}" data-live-field-name="ipor-amount">Withdraw</button>
              </div>
              ${notice ? `<div class="action-notice">${notice}</div>` : ''}
            </div>
      ...
      <div class="left-stack">
        ${renderCard('Developer Simulation', `
          <div class="metric-grid">
            ...
            ${metric('Network', page.network)}
            ${metric('Address', page.contractAddress ? shortAddress(page.contractAddress) : 'Pending')}
