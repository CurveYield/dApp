...
export const BASE_RPC_URL = 'https://base.drpc.org';
...
const IPOR_DATA_API_URL = 'https://api.ipor.io';
...
function formatDisplayUnits(value, decimals, precision = 2) {
  const raw = formatUnits(value, decimals, precision);
  const [whole, fraction = ''] = raw.split('.');
  const grouped = BigInt(whole || '0').toLocaleString('en-US');
  return precision > 0 ? `${grouped}.${fraction}` : grouped;
}

function formatUsdDisplay(value, decimals = 18, precision = 2) {
  return `$${formatDisplayUnits(value, decimals, precision)}`;
}

function formatPercentFromRatio(value, total) {
  const denominator = BigInt(total || 0n);
  if (denominator === 0n) return '0.00%';
  return `${(Number((BigInt(value || 0n) * 10000n) / denominator) / 100).toFixed(2)}%`;
}

function usdValueFromAssetAmount(amount, assetUsdPrice, assetUsdPriceDecimals) {
  return (BigInt(amount || 0n) * BigInt(assetUsdPrice || 0n)) / (10n ** BigInt(assetUsdPriceDecimals || 18));
}

async function fetchJsonWithTimeout(url, timeoutMs = RPC_REQUEST_TIMEOUT_MS) {
...
function buildIporPerformanceHistory(history) {
  const points = (Array.isArray(history) ? history : [])
    .map((entry) => {
      const apy = Number(entry.underlyingAssetApy ?? entry.apr1d ?? entry.apy ?? 0);
      const share = Number(entry.updatedAssetsToSharesRatio ?? entry.assetsToSharesRatio ?? 1);
      const date = entry.blockTimestamp ? new Date(entry.blockTimestamp) : null;
      return {
        apy: Number.isFinite(apy) ? apy : 0,
        share: Number.isFinite(share) && share > 0 ? share : 1,
        label: date && !Number.isNaN(date.getTime())
          ? date.toLocaleDateString('en-US', { month: '2-digit', day: '2-digit', timeZone: 'UTC' })
          : '',
      };
    })
    .filter((entry) => entry.label || entry.apy !== 0 || entry.share !== 1)
    .slice(-24);
  if (points.length === 0) return null;
  const apyValues = points.map((entry) => entry.apy);
  const shareValues = points.map((entry) => entry.share);
  const apyMin = Math.min(-1, ...apyValues);
  const apyMax = Math.max(1, ...apyValues);
  ...
  return {
    ...
    apyMax,
    shareMin,
    shareMax: shareMax === shareMin ? shareMin + 0.01 : shareMax,
    xLabels: [labels[0], labels[Math.floor(labels.length / 3)], labels[Math.floor((labels.length * 2) / 3)], labels[labels.length - 1]].filter(Boolean),
    apyLabels: [`${apyMax.toFixed(2)}%`, `${((apyMin + apyMax) / 2).toFixed(2)}%`, `${apyMin.toFixed(2)}%`],
    shareLabels: [shareMax.toFixed(4), ((shareMin + shareMax) / 2).toFixed(4), shareMin.toFixed(4)],
  };
}

async function fetchIporVaultHistory(page) {
  const chain = numericChainId(page.chainId);
  if (!chain || !page.contractAddress) return null;
  const payload = await fetchJsonWithTimeout(`${IPOR_DATA_API_URL}/fusion/vaults-history/${chain}/${page.contractAddress}`, 8000);
  return buildIporPerformanceHistory(payload?.history);
}
...
function formatCompactUsdValue(value) {
  ...
  const number = Number(absolute) / 1e18;
  if (!Number.isFinite(number)) return `${sign}$0`;
  if (number >= 1_000_000_000) return `${sign}$${(number / 1_000_000_000).toFixed(2).replace(/\.00$/, '')}B`;
  if (number >= 1_000_000) return `${sign}$${(number / 1_000_000).toFixed(2).replace(/\.00$/, '')}M`;
  if (number >= 1_000) return `${sign}$${(number / 1_000).toFixed(2).replace(/\.00$/, '')}K`;
  return `${sign}$${number.toFixed(2).replace(/\.00$/, '')}`;
}

export function formatCapUsagePercent(used, cap) {
  const capValue = BigInt(cap || 0n);
  if (capValue <= 0n) return '0%';
  const bps = (BigInt(used || 0n) * 10000n) / capValue;
  const number = Number(bps) / 100;
  const precision = number >= 10 ? 0 : number >= 1 ? 2 : 2;
  return `${number.toFixed(precision).replace(/\.00$/, '').replace(/(\.\d)0$/, '$1')}%`;
}

export function formatResolvedUsdCap(rawCap, decimals, usdPriceRaw, usedRaw = 0n) {
  const resolved = resolveAmountCap(rawCap);
  if (resolved === null) return '∞';
  const price = BigInt(usdPriceRaw || 0n);
  if (price <= 0n) return formatUnits(resolved, decimals, 2);
  const usdValue = (resolved * price) / (10n ** BigInt(decimals));
  return `${formatCompactUsdValue(usdValue)} (${formatCapUsagePercent(usedRaw, resolved)})`;
}

export function formatOptionalResolvedUsdCap(rawCap, decimals, usdPriceRaw, usedRaw = 0n) {
  if (rawCap === null || rawCap === undefined) return 'Loading...';
  return formatResolvedUsdCap(rawCap, decimals, usdPriceRaw, usedRaw);
}
...
export function apyFromInterestRate(rate) {
  const number = Number(rate || 0n);
  if (!Number.isFinite(number) || number === 0) return '0.00%';
  const secondsPerYear = 31536000;
  const apr = (number * secondsPerYear) / 1e27;
  return `${((Math.exp(apr) - 1) * 100).toFixed(2)}%`;
}
...
export function formatIrmMetrics({ baseRate, slope1, slope2, kink }) {
  ...
  const kinkValue = BigInt(kink || 0n) > UINT32_MAX ? UINT32_MAX : BigInt(kink || 0n);
  const rateAtKink = base + (firstSlope * kinkValue);
  const maxRate = rateAtKink + (secondSlope * (UINT32_MAX - kinkValue));
  return {
    irmBaseRate: apyFromInterestRate(base),
    irmRateAtKink: apyFromInterestRate(rateAtKink),
    irmMaxRate: apyFromInterestRate(maxRate),
    irmKink: formatUint32Percent(kinkValue),
  };
}
...
async function readIporAssetUsdValue(page, vault, rpcUrl) {
  if (!vault?.asset || vault.totalAssets === null) return null;
  const middlewareRaw = page.priceOracleMiddleware
    ? `0x${encodeAddress(page.priceOracleMiddleware)}`
    : await safeEthCall(page.contractAddress, SELECTORS.getPriceOracleMiddleware, rpcUrl, 2, page.chainId);
  if (!middlewareRaw) return null;
  const middleware = page.priceOracleMiddleware || decodeAddress(middlewareRaw);
  const priceAsset = page.priceOracleAssetAddress || vault.asset;
  const priceRaw = await safeEthCall(middleware, encodeCall(SELECTORS.getAssetPrice, [
    encodeAddress(priceAsset),
  ]), rpcUrl, 2, page.chainId);
  if (!priceRaw) return null;
  const assetUsdPrice = decodeUintWord(priceRaw, 0);
  const assetUsdPriceDecimals = Number(decodeUintWord(priceRaw, 1) || 18n);
  const priceScale = 10n ** BigInt(assetUsdPriceDecimals);
  if (priceScale === 0n) return null;
  const totalAssetsUsd = (vault.totalAssets * assetUsdPrice) / priceScale;
  const totalSupplyUsd = vault.totalSupply === null ? null : (vault.totalSupply * assetUsdPrice) / priceScale;
  return {
    priceOracleMiddleware: middleware,
    priceOracleAssetAddress: priceAsset,
    assetUsdPrice: formatUsdDisplay(assetUsdPrice, assetUsdPriceDecimals, 6),
    assetUsdPriceRaw: assetUsdPrice.toString(),
    assetUsdPriceDecimals,
    totalValueManaged: formatDisplayUnits(totalAssetsUsd, vault.decimals, 2),
    totalValueLocked: totalSupplyUsd === null ? null : formatDisplayUnits(totalSupplyUsd, vault.shareDecimals, 2),
  };
}

async function readIporMarketAllocations(page, vault, value, rpcUrl) {
  if (!Array.isArray(page.marketRows) || page.marketRows.length === 0 || vault.totalAssets === null) return null;
  if (!value?.assetUsdPriceRaw || !Number.isFinite(Number(value.assetUsdPriceDecimals))) return null;
  const marketIds = [...new Set(page.marketRows.map((row) => row.marketId).filter(Boolean))];
  const calls = marketIds.map((marketId) => ({
    marketId,
    to: page.contractAddress,
    data: encodeCall(SELECTORS.totalAssetsInMarket, [encodeUint(marketId)]),
  }));
  const results = calls.length
    ? await safeEthBatch(calls.map(({ to, data }) => ({ to, data })), rpcUrl, 1, page.chainId)
    : [];
  const marketAmounts = new Map(calls.map((call, index) => [call.marketId, results[index] ? decodeUint(results[index]) : 0n]));
  const uniqueAllocated = [...marketAmounts.values()].reduce((sum, amount) => sum + amount, 0n);
  const remainder = vault.totalAssets > uniqueAllocated ? vault.totalAssets - uniqueAllocated : 0n;
  const price = BigInt(value.assetUsdPriceRaw);
  const priceDecimals = value.assetUsdPriceDecimals;
  const nextRows = page.marketRows.map((row) => {
    const amount = row.remainderOfTotal ? remainder : (row.marketId ? marketAmounts.get(row.marketId) || 0n : null);
    if (amount === null) return row;
    const usd = usdValueFromAssetAmount(amount, price, priceDecimals);
    const percent = formatPercentFromRatio(amount, vault.totalAssets);
    return {
      ...row,
      assets: formatUsdDisplay(usd, vault.decimals, usd >= 1000n * (10n ** BigInt(vault.decimals)) ? 0 : 2),
      caption: row.remainderOfTotal || row.marketId ? percent : row.caption,
      assetsRaw: amount.toString(),
      assetsUsdRaw: usd.toString(),
      allocation: percent,
    };
  });
  const allocationRows = nextRows.filter((row) => row.allocationChart !== false && (row.remainderOfTotal || row.marketId));
  return {
    marketRows: nextRows,
    allocationHistory: {
      labels: allocationRows.map((row) => row.label),
      values: allocationRows.map((row) => Number(row.assetsUsdRaw || '0') / Number(10n ** BigInt(vault.decimals))),
      maxValueLabel: value.totalValueManaged ? `$${value.totalValueManaged}` : '100%',
      midValueLabel: '50%',
      minValueLabel: '$0',
    },
  };
...
async function readEarnAllocations(page, earnVault, assetDecimals, priceRaw, rpcUrl) {
  ...
  const marketMetricCalls = vaultAddresses.flatMap((vaultAddress) => ([
    { to: vaultAddress, data: SELECTORS.interestRate },
    { to: vaultAddress, data: SELECTORS.totalBorrows },
    { to: vaultAddress, data: SELECTORS.totalAssets },
    { to: vaultAddress, data: SELECTORS.interestFee },
  ]));
  const marketMetricResults = await resilientEthBatch(marketMetricCalls, rpcUrl, 2, page.chainId);
  ...
  return rows.map((row, index) => {
    ...
    const metricOffset = index * 4;
    const interestRateRaw = marketMetricResults[metricOffset];
    const totalBorrowsRaw = marketMetricResults[metricOffset + 1];
    const totalAssetsRaw = marketMetricResults[metricOffset + 2];
    const interestFeeRaw = marketMetricResults[metricOffset + 3];
    const totalBorrows = totalBorrowsRaw ? decodeUint(totalBorrowsRaw) : 0n;
    const totalAssets = totalAssetsRaw ? decodeUint(totalAssetsRaw) : 0n;
    const interestFee = interestFeeRaw ? decodeUint(interestFeeRaw) : 0n;
    const borrowApy = interestRateRaw ? apyFromInterestRate(decodeUint(interestRateRaw)) : '0.00%';
    const utilization = formatUtilization(totalBorrows, totalAssets);
    const supplyApy = supplyApyFromBorrow({ borrowApy, utilization, interestFee });
...
export async function fetchLivePageMetrics(page, rpcUrl = null) {
  ...
  if (page.type === 'ipor-vault') {
    ...
    const totalSupply = vault.totalSupply === null ? page.totalSupply || '0.00' : formatDisplayUnits(vault.totalSupply, vault.shareDecimals, 2);
    const totalAssets = vault.totalAssets === null ? page.totalAssets || '0.00' : formatDisplayUnits(vault.totalAssets, vault.decimals, 2);
    const value = await readIporAssetUsdValue(page, vault, rpcUrl).catch(() => null);
    const allocations = await readIporMarketAllocations(page, vault, value, rpcUrl).catch(() => null);
    const performanceHistory = await fetchIporVaultHistory(page).catch(() => null);
    const liveUsdValue = value?.totalValueManaged || null;
    const liveUsdLocked = value?.totalValueLocked || null;
    const useIporValueFallback = page.valueDisplaySource === 'ipor-page';
    const capacity = {};
    if (page.capacityMaxTokens && vault.totalSupply !== null) {
      const maxCapacity = parseUnits(page.capacityMaxTokens, vault.shareDecimals);
      const remaining = maxCapacity > vault.totalSupply ? maxCapacity - vault.totalSupply : 0n;
      const percent = maxCapacity > 0n ? Number((vault.totalSupply * 10000n) / maxCapacity) / 100 : 0;
      capacity.capacityRemaining = `${formatDisplayUnits(remaining, vault.shareDecimals, 0)} ${page.asset} left before reaching maximum capacity`;
      capacity.depositsLabel = `Deposits: ${formatDisplayUnits(vault.totalSupply, vault.shareDecimals, 0)} ${page.asset}`;
      capacity.maxCapacityLabel = `Max capacity: ${formatDisplayUnits(maxCapacity, vault.shareDecimals, 0)} ${page.asset}`;
      capacity.capacityPercent = Math.max(0, Math.min(100, percent));
    }
    return {
      assetAddress: vault.asset,
      totalSupply,
      totalAssets,
      totalAssetsUnit: `${totalAssets} ${page.asset}`,
      totalLockedUnit: `${totalSupply} ${page.asset}`,
      totalValueManaged: liveUsdValue || (useIporValueFallback ? page.totalValueManaged || totalAssets : totalAssets),
      totalValueLocked: liveUsdLocked || (useIporValueFallback ? page.totalValueLocked || totalSupply : totalSupply),
      availableLiquidity: vault.totalAssets === null ? page.totalValueLocked || '0.00' : totalAssets,
      supplyApy: page.strategyApr || '9.33%',
      performanceApy: page.strategyApr || '9.33%',
      performanceHistory: performanceHistory || page.performanceHistory,
      ...value,
      ...allocations,
      ...capacity,
    };
  ...
  if (page.type === 'earn') {
    ...
    const earnSupplyApy = calculateWeightedEarnSupplyApy(earnAllocations, vault.totalAssets ?? 0n);
    return {
      assetAddress: vault.asset,
  ...
  let [borrowLtv, liquidationLtv, ltvFullRaw, capsRaw, collateralCapsRaw, convertRaw, collateralConvertRaw, feeReceiverRaw, interestFeeRaw, irmRaw, oracleRaw, unitRaw, hookRaw, governorRaw, debtTokenRaw] = await safeEthBatch([
    ...
    { to: debtVaultAddress, data: SELECTORS.governorAdmin },
    { to: debtVaultAddress, data: SELECTORS.dToken },
  ], rpcUrl, 2, page.chainId);
  ...
  if (!governorRaw) governorRaw = await safeEthCall(debtVaultAddress, SELECTORS.governorAdmin, rpcUrl, 2, page.chainId);
  if (!debtTokenRaw) debtTokenRaw = await safeEthCall(debtVaultAddress, SELECTORS.dToken, rpcUrl, 2, page.chainId);
  ...
  const debtUsdPriceRaw = debtPriceRaw ? decodeUint(debtPriceRaw) : 0n;
  let collateralUsdPriceRaw = 0n;
  try {
    collateralUsdPriceRaw = debtUsdPriceRaw && price && !String(price).includes('Loading')
      ? (parseUnits(price, debtVault.decimals) * debtUsdPriceRaw) / (10n ** BigInt(debtVault.decimals))
      : 0n;
  } catch {
    collateralUsdPriceRaw = 0n;
  }

  const supplyApy = '0.00%';
  const borrowApy = apyFromInterestRate(debtVault.interestRate);
  ...
  return {
    ...
    debtTokenAddress: debtTokenRaw ? decodeAddress(debtTokenRaw) : null,
    price: price || 'Loading...',
    collateralPrice: collateralUsdPriceRaw > 0n ? `$${formatUnits(collateralUsdPriceRaw, 18, 4)}` : 'Loading...',
    debtPrice: debtUsdPriceRaw > 0n ? `$${formatUnits(debtUsdPriceRaw, 18, 4)}` : '$1.00',
    ...
    utilization,
    supplyCap: ltvConfigured ? formatOptionalResolvedUsdCap(supplyCapRaw, debtVault.decimals, debtUsdPriceRaw, debtVault.totalSupply) : 'Not configured',
    collateralSupplyCap: ltvConfigured ? formatOptionalResolvedUsdCap(collateralSupplyCapRaw, collateralVault.decimals, collateralUsdPriceRaw, collateralVault.totalSupply) : 'Not configured',
    borrowCap: ltvConfigured ? formatOptionalResolvedUsdCap(borrowCapRaw, debtVault.decimals, debtUsdPriceRaw, debtVault.totalBorrows) : 'Not configured',
    ...
    feeReceiver: feeReceiverRaw ? decodeAddress(feeReceiverRaw) : null,
    riskManager: governorRaw ? decodeAddress(governorRaw) : null,
