...
Run the balYIELD Base Phase 1 app:
...
```powershell
powershell -ExecutionPolicy Bypass -File .\run-bal-dev.ps1
```
...
The balYIELD pool explorer reads `apps/frontend-v3/public/data/balyield-pools.json` instead of Balancer's public API because balYIELD pools are deployed through balYIELD factory wrappers and are not expected to appear in Balancer's indexed pool list.

Verify the registry and direct router quote:

```powershell
node tools\verify-balyield-phase1.mjs
```
